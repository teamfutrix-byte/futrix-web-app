...
function getXPColumn(sheet) {
  ...
  for (var i = 0; i < headers.length; i++) { if (String(headers[i]).trim() === 'XP Points') return i + 1; }
  var newCol = lastCol + 1; sheet.getRange(1, newCol).setValue('XP Points'); sheet.getRange(1, newCol).setFontWeight('bold'); return newCol;
}
...
function verifyLogin(e) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][2]).toLowerCase().trim() === email && String(data[i][3]).trim() === phone) {
      var xp = parseFloat(data[i][xpCol - 1]) || 0;
      if (xp === 0) { xp = 100; sheet.getRange(i + 1, xpCol).setValue(100); }
      return { success: true, name: String(data[i][1]).trim(), email: email, phone: phone, xp: xp };
    }
  }
...
function saveResponse(e) {
  ...
  sheet.appendRow([new Date(), e.parameter.candidateName||'', e.parameter.candidateEmail||'', e.parameter.candidatePhone||'',
  ...
  if (xpEarned !== 0) {
    ...
    var uEmail = (e.parameter.candidateEmail||'').toLowerCase().trim();
    for (var i = 1; i < regData.length; i++) {
      if (String(regData[i][2]).toLowerCase().trim() === uEmail) {
...
function checkAttempt(e) {
  ...
  // Column C = Email (index 2), Column E = Series ID (index 4)
  for (var i = 1; i < data.length; i++) {
    var rowEmail  = String(data[i][2] || '').toLowerCase().trim();
    var rowSeries = String(data[i][4] || '').trim();
...
function checkRegistration(e) {
  ...
  // Column C (index 2) = Email, Column D (index 3) = Phone
  for (var i = 1; i < data.length; i++) {
    var rowEmail = String(data[i][2] || '').toLowerCase().trim();
